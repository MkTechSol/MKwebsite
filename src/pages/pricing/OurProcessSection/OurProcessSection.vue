<template>
  <section class="py-16 px-4 bg-white text-center mt-16">
    <div class="max-w-6xl mx-auto">
      <!-- Section Header -->
      <p class="inline-block mb-2 px-4 py-2 bg-[#E3EAFD] text-[var(--color-accent-text)] font-semibold rounded-md text-sm animation-fadeup animation-fadeup">
        Process
      </p>
      <h2 class="text-3xl md:text-4xl font-bold mb-12 animation-fadeup animation-fadeup">
        Our <span class="text-[var(--color-red-text)]">Process</span>
      </h2>

      <!-- Process Cards -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div v-for="(step, index) in steps" :key="index" class="bg-white rounded-xl overflow-hidden shadow-sm animation-fadeup">
          <img :src="step.image" :alt="step.title" class="w-full h-60 object-cover" />
          <div class="p-6 text-left">
            <h3 class="text-[#4EA5F2] font-bold text-base mb-3 ">{{ step.title }}</h3>
...
const steps = [
  {
    ...
    points: [
      ...
      'Provide feedback for necessary changes, if any.'
    ]
  },
  {
  ...
  },
  {
    ...
    points: [
      ...
      'Ensure app remains live and functional across OS updates.'
    ]
  }
]
</script>
